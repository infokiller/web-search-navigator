 ...
 * Contexts :
 *
 * Some properties can be set on specified context.
 ...
 *      When set, the closest parent element of the focused one matching this
 *      Selector will be highlighted
 *  - {number} marginTop, Set this if top results are not entirely visible
 *  - {number} marginBottom, Set this if bottom results are not entirely
 *    visible
 *
 */
...
const searchEngines = [
  ...
  {
    /**
         * @return {boolean} true if result navigation keybindings can be
    ...
    */
    ...
    /**
     * Get result links
     * @return a SearchResultCollection object
     */
    getSearchLinks() {
      return new SearchResultCollection(
          [
            ...
            // Top stories
            // [docment.querySelectorAll('div.mCBkyc'), null],
            // Top stories, Twitter, and videos.
            // [document.querySelectorAll('[data-init-vis=true] [role=heading]'), null],
  ...
  // Startpage
  {
    ...
    contexts: {
      ...
      videos: () => {
        const vidsLayoutDiv = 
            document.querySelector('div.layout-video.layout-video--default');
  ...
  },
  {
    ...
    changeTools(period) {
      if (!document.querySelector('div#collapse-content')) {
        const toggle_btn = document.querySelectorAll('a.ytd-toggle-button-renderer')[0];
